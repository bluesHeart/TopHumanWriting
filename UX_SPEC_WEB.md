...
## 1) Product Snapshot
- 用户：研究者/学生/编辑（中英写作），有自己的“本地论文范文库”
- 核心诉求：把稿子改得**更像范文写法**，并且是**白箱**（每条建议/改写都能追溯到范文证据）
- 主要约束：8GB Windows 笔记本；50–100 PDF；离线优先（检索 + 本地 LLM）
- 成功指标：
...
- Page map：
  ...
  - `对齐润色`：范文对照（C1..Ck）→ 生成白箱诊断 + 轻改/中改（Qwen）
  - `本地 LLM`：一键启动&测试 / 8GB 预设 / 停止 / 打开模型目录
  - `使用帮助`：快速上手 + “Qwen 的作用在哪里”
...
- Steps：
  1) 获取范文对照（C1..Ck）
  2) 生成对齐润色（Qwen）：输出 JSON → 解析展示
- Output：
  - 白箱诊断：每条必须包含 evidence（quote 是范文原文子串）
  - 两档改写：`light` / `medium`，并列出 changes + citations

### Flow D: Local LLM “Is it working?”
- Entry：本地 LLM 页 → 8GB 预设 → 一键启动&测试
- Output：明确显示 server/model 是否存在、是否 running、base_url
...
- `本地 LLM`
  - 组件：路径显示；8GB 预设；一键测试；停止；打开模型目录；状态表
  - 状态：缺少 server/model；测试失败提示
...
- 本地 LLM：一键测试返回 OK；状态能显示 running/base_url；停止后 running=false
